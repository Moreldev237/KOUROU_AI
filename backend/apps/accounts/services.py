...
def generate_otp_code(user, purpose: str) -> OTPCode:
    ...
    # Pour l'inscription, on envoie toujours l'OTP par e-mail.
    # Le projet ne supporte plus l'envoi d'OTP par SMS.
    if purpose == OTPPurpose.REGISTRATION:
        if not user.email:
            logger.warning("Registration OTP requested but user has no email: %s", user)
        else:
            send_email(
                to=user.email,
        ...
        # Les autres usages (ex: password_reset) restent envoyés par e-mail.
        send_email(
            to=user.email,
            subject="KOUROU AI - Vérification",
            message=(
                f"Bonjour {user.full_name},\n\n"
                f"Votre code de vérification est : {code}\n\n"
                f"Il expire dans {OTP_VALIDITY_MINUTES} minutes.\n\n"
