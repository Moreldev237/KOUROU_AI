...
from apps.ai_engine.models import TutorConversation

logger = logging.getLogger("apps")


@shared_task
def delete_expired_tutor_conversations_task():
    cutoff = timezone.now() - timedelta(hours=48)
    deleted, _ = TutorConversation.objects.filter(updated_at__lt=cutoff).delete()
    logger.info("Conversations du tuteur supprimées après 48 h : %s objet(s).", deleted)
    return deleted
