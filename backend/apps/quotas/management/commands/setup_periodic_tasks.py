...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        cleanup_schedule, _ = CrontabSchedule.objects.get_or_create(
            minute="30", hour="0", day_of_week="*", day_of_month="*", month_of_year="*", timezone="Africa/Douala"
        )
        cleanup_task, cleanup_created = PeriodicTask.objects.update_or_create(
            name="Suppression des conversations du tuteur après 48 heures",
            defaults={
                "crontab": cleanup_schedule,
                "task": "apps.ai_engine.tasks.delete_expired_tutor_conversations_task",
                "enabled": True,
            },
        )
        cleanup_verb = "créée" if cleanup_created else "mise à jour"
        self.stdout.write(
            self.style.SUCCESS(f"Tâche planifiée {cleanup_verb} : « {cleanup_task.name} » (tous les jours à 00:30, Africa/Douala).")
        )
